namespace DeepLClient.Functions
{
    ...
    internal static class ComboBoxTheme
    {
        ...
        /// <summary>
        /// Converts the items to KeyValuePair[string, string] and draws the key value
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        internal static void DrawDictionaryStringStringItemUsingKey(object sender, DrawItemEventArgs e)
        {
            // only relevant for listviews in detail mode
            if (sender is not ComboBox comboBox) return;
            ...
            if (index != -1)
            {
                ...
                // draw the string
                var brush = (e.State & DrawItemState.Selected) > 0 ? new SolidBrush(Color.FromArgb(63, 63, 70)) : new SolidBrush(comboBox.ForeColor);
                e.Graphics.DrawString(value.Key, Variables.DefaultFont, brush, e.Bounds, StringFormat.GenericDefault);
            }
